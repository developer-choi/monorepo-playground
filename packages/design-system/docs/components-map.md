...
- 상태: 신규 (DS `Modal`/`Dialog` 위에 얹음)
- 상황: Alert(확인 1버튼, void) / Confirm=`ConfirmDialog`(취소·확인 2버튼, `overlay.openAsync`로 boolean await) / Form(폼+제출/취소). themes `AlertDialog` 실사용처(게시판 삭제 확인)가 Confirm 케이스.
- 참고: `apps/examples/docs/patterns/overlay/OverlayKitModal.md`(overlay-kit 명령형 호출 — `ConfirmDialog`는 이 문서에 이름만 있고 미구현), DS `src/components/Modal/`
...
- **Board\***: Card/Form/Table/Detail/Filter/ListPage/Skeleton/TagInput — Card/Table/Badge/Separator/ConfirmDialog 조합 게시판 도메인.
