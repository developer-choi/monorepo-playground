...
- [디자인 시스템](https://github.com/developer-choi/monorepo-playground/pull/2) — 디자이너와 Material Design 기반으로 미래 변경까지 고려한 스펙을 잡고, MUI 원본 코드를 AI로 분석하여 설계 ([기술적 의사결정과정](packages/design-system/README.md#기술적-의사결정과정))
...
- **자동 포맷팅** — Prettier가 lint-staged와 결합되어 커밋 시 변경 파일만 자동으로 정리합니다.
  스타일 차이로 PR diff가 노이즈로 차는 일이 사라지고, 사람과 AI 모두 포맷에 신경 쓰지 않아도 됩니다.
- **커밋 메시지 게이트** — commitlint가 커밋 메시지의 형식과 한글 본문을 강제합니다.
  일관된 히스토리는 git log와 blame을 잡음 없이 읽게 만들고, 이후 회고와 변경 이력 추적의 기반이 됩니다.
- **Git Hooks 2단계** — pre-commit에서는 변경된 파일만 빠르게 검사해 커밋 속도를 확보합니다.
  pre-push에서는 전체 코드를 대상으로 타입 검사와 정적 분석을 돌려 안전을 보장합니다.
