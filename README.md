...
- [최고의 무한스크롤이란 무엇인가](docs/guides/infinite-scroll/step1.md) — 렌더링 전략, 이미지 최적화, 무한스크롤, 에러 방어까지 4단계로 정리
- [검색 결과 UX 최적화](https://github.com/developer-choi/monorepo-playground/pull/3) — useDeferredValue로 이전 결과를 유지하면서 자연스럽게 전환
- [디자인 시스템](packages/design-system/README.md) — 디자이너·Material Design 기반 스펙 설계부터 MUI 원본 분석까지, [표준화→추상화→headless→모노레포 구축기](packages/design-system/docs/guides/design-system/step1.md)

### 잘 리뷰하기 위한 best practice

- [정적 분석](https://github.com/developer-choi/monorepo-playground/pull/11) — ESLint·Stylelint로 await 누락, switch case 누락, 매직 넘버, 하드코딩 색상 같은 실수를 커밋 시점에 차단합니다.
  타입 안전을 우회하는 `as any`, `ts-ignore` 같은 패턴도 막아 AI가 만든 회피성 코드를 사전에 거릅니다.
- [테스트 코드](https://github.com/developer-choi/monorepo-playground/blob/master/docs/guides/testing/why-to-test.md) — 기능이 바뀌거나 AI가 코드를 수정할 때마다, 기존 동작이 깨지지 않았는지 자동으로 확인합니다.
  사람이 직접 검증해야 할 범위가 줄어들고, 리뷰어는 로직과 설계에만 집중할 수 있습니다.
...
구축 과정은 [디자인 시스템 구축기](packages/design-system/docs/guides/design-system/step1.md)에서 확인할 수 있습니다.
