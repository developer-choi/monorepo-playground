...
- **지금은 에러가 나지 않아야** 합니다 — 기존 코드가 빌드/커밋을 차단하면 안 됨
- **파일을 수정할 때 에러가 반드시 나야** 합니다 — 수정 기회에 정리를 강제
- **작업 범위를 크게 벗어나지 않아야** 합니다 — 한 줄 고치려다 파일 전체를 리팩토링하게 되면 안 됨
...
- `.githooks/pre-commit` — `check-file-level-disable.sh`를 lint-staged 전에 실행
