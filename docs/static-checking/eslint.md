...
#### 2. `@typescript-eslint/return-await` (in-try-catch)
...
#### 3. `@typescript-eslint/switch-exhaustiveness-check`
...
#### 4. `@typescript-eslint/no-misused-promises` (checksVoidReturn.attributes: false)
...
#### 6. `@typescript-eslint/no-unnecessary-condition`
...
#### 9. `@typescript-eslint/restrict-template-expressions` (allowNullish: false)
