...
하드코딩된 색상·수치를 변수 사용으로 강제하는 플러그인.

### 도입 히스토리

처음에는 `stylelint-declaration-strict-value`의 두 가지 한계 때문에 커스텀 Stylelint 플러그인(JS)을 직접 작성했다.

1. **`ignoreFunctions` all-or-nothing** — `false`로 설정하면 `rgb()`뿐 아니라 `calc()`, `clamp()`, `min()`, `max()`까지 모두 차단됨. 색상 함수만 선별 차단 불가
2. **longhand 없는 shorthand 미분해** — `box-shadow`, `text-shadow`는 대응하는 longhand가 없어서 `expandShorthand`가 작동하지 않음. 첫 토큰만 검사하고 나머지(rgba, named color 등) 통과

커스텀 플러그인은 `postcss-value-parser`로 모든 토큰을 개별 순회하여 이 문제를 해결했으나, JS 파일 직접 관리 부담이 발생. npm 정식 플러그인으로 전환하고 shadow 속성의 한계는 수용하기로 결정.

### 수용하는 한계

`box-shadow`, `text-shadow`에서 하드코딩된 색상을 감지하지 못한다.
...
shadow 속성은 속성 목록에 포함하되, 잡을 수 있는 케이스(단일 값, 변수 없이 하드코딩)만 기대한다.
