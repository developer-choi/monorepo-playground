...
`.prettierignore`: 빌드 산출물(`dist`, `build`, `.next`), 의존성(`node_modules`), lock 파일 등 자동 생성 파일과 `docs/`·`templates/`를 포맷팅 대상에서 제외합니다. `docs/`는 사람이 직접 작성하는 문서 영역이며 ESLint/tsc 검사 대상도 아니므로, `prettier --write`가 자동으로 다시 정렬해 의도하지 않은 대규모 diff를 만들지 않도록 ignore에 둡니다. `templates/`는 단일레포 채용과제용으로 평탄화한 설정 복사본이라 MP 포맷 대상이 아닙니다. lint-staged의 `*.md` 글롭에 docs 안의 파일이 걸려도 Prettier가 `.prettierignore`를 자동 존중해 건너뜁니다.
