...
`.prettierignore`: 빌드 산출물(`dist`, `build`, `.next`), 테스트 산출물(`coverage`, `playwright-report`, `test-results`), 의존성(`node_modules`), lock 파일 등 자동 생성 파일과 `docs/`·`templates/`를 포맷팅 대상에서 제외합니다. 테스트 산출물은 `.gitignore`에도 있어 커밋되진 않지만, `prettier --check .`는 워킹 트리를 그대로 훑기 때문에 여기서 빼주지 않으면 E2E를 한 번 돌린 뒤부터 `format:check`가 실패합니다. `docs/`는 사람이 직접 작성하는 문서 영역이며 ESLint/tsc 검사 대상도 아니므로, `prettier --write`가 자동으로 다시 정렬해 의도하지 않은 대규모 diff를 만들지 않도록 ignore에 둡니다. `templates/`는 단일레포 채용과제용으로 평탄화한 설정 복사본이라 MP 포맷 대상이 아닙니다. lint-staged의 `*.md` 글롭에 docs 안의 파일이 걸려도 Prettier가 `.prettierignore`를 자동 존중해 건너뜁니다.
...
lint-staged는 glob 패턴별로 매칭한 staged 파일에 아래 명령들을 실행합니다. 기본값은 glob 그룹을 병렬 실행하지만, 겹치는 glob이 같은 파일을 동시에 수정하는 경쟁 조건(Prettier와 ESLint가 같은 `.ts`에 동시 쓰기 → 한쪽 수정 유실)을 막기 위해 `--concurrent false`로 **순차 실행**합니다:
