...
function useSomeForm() {
  ...
  const passwordInputProps: InputProps = {
    ...register('password', {
      required: '비밀번호를 입력해주세요', // [ai-only] 필수인 경우. 추가 규칙은 필요한 경우 작성
    }),
    label: '비밀번호',
  ...
  const onSubmit: SubmitHandler<SomeFormData> = async (data) => {
    ...
    try {
      ...
      // [ai-only] 개별 에러를 먼저 분기, 나머지는 공통 함수에 위임
  ...
  /* [ai-only] 반환 구조: 영향 범위 기준으로 그룹화한다.
