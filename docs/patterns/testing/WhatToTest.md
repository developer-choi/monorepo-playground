...
- 그 외는 Integration 기본
- E2E는 **아래 레벨로 같은 결과를 못 얻는 것만** 남긴다 — 전체가 붙어야만 드러나는 흐름

### 같은 확신이면 낮은 레벨로 내린다

레벨을 올릴수록 확신은 오르지만 속도가 떨어지고, 깨졌을 때 원인 범위가 넓어진다. **같은 버그가 잡힌다면 위 레벨을 쓸 이유가 없다.**

| 레벨 | 확신 | 속도 | 깨졌을 때 원인 범위 |
|---|---|---|---|
| E2E | 높음 | 느림 (브라우저·서버 기동, flaky) | 전 계층 |
| Integration | 중간 | 빠름 | 렌더한 트리 안쪽 |
| Unit | 낮음 | 가장 빠름 | 그 함수 하나 |

이메일 형식 검증처럼 함수 하나로 판정이 끝나는 것을 E2E로 쓰면, 확신은 거의 같은데 케이스마다 브라우저 왕복을 반복하고 디버깅 범위만 넓어진다 → Unit.

E2E가 가장 많은 **icecream cone은 오답**이다. 한 페이지의 케이스가 E2E 파일에 몰리면 위 손해를 그대로 다 받는다. 피라미드(Unit 최다)든 트로피(Integration 최다)든 무방하다.

> 근거·예제 전개(채용담당자용 긴 글): [guides/testing/test-levels.md](../../guides/testing/test-levels.md). AI는 이 요약으로 판단하고, 이 링크는 열지 않는다.
