...
`gcTime: 30_000`은 두 방향의 trade-off에서 고른 값입니다. 더 짧으면 잠깐 다른 페이지를 봤다 돌아와도 캐시가 비워져 1페이지부터 다시 로드되고, 더 길면 오래된 목록을 보여주거나 메모리를 그만큼 더 오래 점유합니다.
...
### memo가 치른 비용, 그리고 캐싱과의 맞물림

`React.memo`는 공짜가 아닙니다. 매 렌더마다 이전 props와 새 props를 얕게 비교하는 비용이 듭니다. 카드에 넘기는 props 중 함수·객체가 매번 새 참조로 만들어지면 이 비교가 항상 실패해, 리렌더링은 그대로 일어나면서 비교 비용만 얹힙니다.

여기서 memo가 듣는 이유는 `BoardCard`에 넘기는 props가 `board` 객체 하나뿐이고, 이동을 `onClick` 콜백이 아니라 `<Link href>`로 처리해 매 렌더마다 새로 만들어지는 함수 prop이 없기 때문입니다.

게다가 이 `board` 참조의 안정성은 위의 `staleTime: Infinity`에 기대고 있습니다. 페이지 데이터가 리페칭되면 `data.pages.flatMap(...)`이 새 `board` 객체를 만들어 참조가 바뀌고 memo가 깨집니다. 즉 캐싱 전략과 memo 최적화는 따로 떨어진 것이 아니라 한 묶음으로 맞물려 있습니다.
