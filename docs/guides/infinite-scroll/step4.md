...
1. **서버 부하 가중**: 이미 장애가 발생한 서버에 수백 건의 요청이 추가로 몰리면, 장애가 더 악화될 수 있습니다. 단일 사용자의 브라우저가 서버에 DDoS와 비슷한 효과를 만들어내는 셈입니다.
...
에러 처리에 대한 보다 체계적인 접근 방식은 [에러 처리는 왜 필요한가?](../error-handling/step1.md)에서 별도로 정리했습니다.
