...
- **sizes를 문자열로 직접 작성해야 함**: `sizes="(max-width: 1023px) 50vw, 25vw"`처럼 미디어쿼리 기반 문자열을 레이아웃에 맞게 수동으로 작성해야 합니다. 이 문자열은 CSS와 따로 관리되므로, 레이아웃이 바뀌면 값이 조용히 틀려도 에러 없이 잘못된 크기의 이미지를 계속 내려받습니다.
