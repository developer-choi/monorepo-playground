...
test('이메일이 비어 있으면 에러 메시지가 뜬다', ...);
test('비밀번호가 8자 미만이면 에러 메시지가 뜬다', ...);
test('비밀번호에 특수문자가 없으면 에러 메시지가 뜬다', ...);
test('비밀번호 표시 버튼을 누르면 입력값이 보인다', ...);
test('아이디 저장을 체크하고 다시 오면 이메일이 채워져 있다', ...);
test('비밀번호 찾기를 누르면 해당 페이지로 이동한다', ...);
```
