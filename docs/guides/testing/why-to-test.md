...
이 시리즈의 첫 단계로, 테스트 코드를 왜 짜는지부터 정합니다. 결론은 **사용자가 이용할 때 문제없다**는 확신을 얻기 위해서입니다.
...
그럼 그 확신을 어느 레벨로 얻나요? → [어느 레벨로 테스트를 작성해야 할까요?](./test-levels.md)