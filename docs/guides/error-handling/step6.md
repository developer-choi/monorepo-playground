...
각 방식은 쓰임이 갈리는 만큼 약점도 분명합니다.

- 모달은 흐름을 멈추므로, 가벼운 알림에 쓰면 사용자를 불필요하게 막습니다.
- 토스트는 잠깐 떴다 사라지므로, 사용자가 놓치면 다시 확인할 수 없습니다. 반드시 조치가 필요한 에러에는 부적합합니다.
- 인풋 메시지는 특정 필드에 묶이므로, 어느 필드와도 연결되지 않는 전역 에러(네트워크 끊김 등)에는 붙일 자리가 없습니다.
